...
class PingPongEnv(utils.EzPickle):

    def __init__(self, frame_skip, max_timestep=1500, log_dir='', seed=None):
        self.frame_skip = frame_skip

        model_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'pingpong.xml')
    ...
    def _get_obs(self):
        ...
        obj_com = self.get_body_com(self.obj_name)
        return np.concatenate([
            obj_com,
            np.cos(actuator_pos),
            np.sin(actuator_pos),
