...
class PPO(nn.Module):
    ...
    '''
    def forward(self, ob, ac, done, atarg, _return, lr_mult):
    ...
    def run(self):
        ...
        while True:
            ...
            ob, ac, done, atarg, tdlamret = segment["ob"], segment["ac"], segment['done'], segment["adv"], segment["tdlamret"]
            vpredbefore = segment["vpred"] # predicted value function before udpate
            atarg = (atarg - atarg.mean()) / atarg.std() # standardized advantage function estimate
            d = Dataset(dict(ob=ob, ac=ac, done=done, atarg=atarg, vtarg=tdlamret), shuffle=not self.pi.recurrent)
            ...
            for _ in range(self.optim_epochs):
                ...
                self.oldpi.reset(batch_size=self.optim_batchsize)
                self.pi.reset(batch_size=self.optim_batchsize)
                for batch in d.iterate_once(self.optim_batchsize):
                    ...
                    total_loss, *newlosses = self.forward(batch["ob"], batch["ac"], batch["done"], batch["atarg"], batch["vtarg"], cur_lrmult)
                    total_loss.backward()
            ...
            for batch in d.iterate_once(self.optim_batchsize):
                # batch['ob'] = rearrange_batch_image(batch['ob'])
                batch = self.convert_batch_tensor(batch)
                _, *newlosses = self.forward(batch["ob"], batch["ac"], batch["done"], batch["atarg"], batch["vtarg"], cur_lrmult)
                losses.append(torch.stack(newlosses[0], dim=0).view(-1))
            mean_losses = torch.mean(torch.stack(losses, dim=0), dim=0).data.cpu().numpy()
            logger.log(fmt_row(13, mean_losses))
    ...
    def traj_segment_generator(self, pi, env, horizon):
        ...
        while True:
            ...
            if done:
                ...
                # terminal value
                yield {"ob": obs, "rew": rews, "vpred": vpreds, "done": dones,
                       "ac": acs, "prevac": prevacs, "nextvpred": vpred * (1 - done),
